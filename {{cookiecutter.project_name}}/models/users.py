...
from peewee import CharField, IntegerField, DateField, TimeField
...
class User(BaseModel):
    ...
    def single_by_phone(cls, phone: int = 0):
        ...
        if phone != 0:
            db = db.where(User.id == phone)

        return model_to_dict(db)

    @classmethod
    def fetch_all(cls, page: int = 1, page_size: int = 10):
        db = User.select(User.name, User.email, User.phone, User.username, User.avatar, User.created_at, User.deleted_at)
